...
| **Border crossing** | Lineage or a `relates` edge reaching outside a project's tree — legal, honest, and declared in the project's generated `INDEX.md` as the exact list of what a copied project directory would leave behind. | ADR 0033, `projects/README.md` |
...
| **Project** | A **directory that scopes records**: `projects/NNNN-slug/` holding `project.md` (identity), a generated `INDEX.md`, its own `ideas/`, and its own `exports/` (ADR 0033). | The structural change the previous edition of this row said would need T's word — T gave it (2026-09-03). Scope is location, ids stay global, root `ideas/` still holds unscoped records. `project.md` is identity, not a record: project-level decisions live as `Decision` artifacts on a record whose subject is the project. |
