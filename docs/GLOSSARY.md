...
| **The Steward** | The persona the main session enters by invoking `/start` (ADR 0031): front door, writer of state, and the performing voice of every inline verb. Dispatches only the fresh-eyes and quarantine exceptions. | `AGENTS.md`, `system/STEWARD.md` |
...
| **Border crossing** | Lineage or a `relates` edge reaching outside a project's tree — legal, honest, and declared in the project's generated `INDEX.md` as the exact list of what a copied project directory would leave behind. In a scoped session a crossing is *named, never followed*, without T's word. | ADR 0033, ADR 0035, `projects/README.md` |
| **Project ADR** | A project's own decision record — `projects/NNNN-slug/docs/adr/NNNN-slug.md`, numbered per project, written by `decide` when the subject is the project rather than one record. Immutable once accepted, superseded never edited, and **read by no other project**. The root `docs/adr/` remains exclusively the machinery's log, authored outside the Steward context. | ADR 0035, `templates/project-adr.md` |
...
| **Project** | A **directory that scopes records**: `projects/NNNN-slug/` holding `project.md` (identity), a generated `INDEX.md`, its own `ideas/`, `exports/`, `inbox/`, and `docs/adr/` (ADR 0033, 0035). | The structural change the previous edition of this row said would need T's word — T gave it (2026-09-03). Scope is location, ids stay global, root `ideas/` still holds unscoped records. Sessions scope to a project via `/start <project>`, and sibling projects never read each other. |
...
| **Target** | The loop a `build-plan` payload is rendered for: `nexus`, or `none` for a plain plan. The switch for "include the Nexus worker loop?", asked of T. Forked under T's org, pinned by tag, fetched by one adopt command — never vendored into the estate, never left in the recipient. | `system/TYPES.md`, ADR 0029, ADR 0030 |
| **Warm adopt path** | Adoption of the kit around a payload whose decisions are already made: `npx … idea-Nexus#<tag> adopt` copies without overwriting, sweeps placeholders from `nexus.adopt.json`, and files what it cannot resolve as `[needs-user-call]`. | ADR 0030 |
