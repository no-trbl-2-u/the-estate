...
| **Stale Seed / reconciliation** | A Seed whose `origin:` state is behind its record's head. The record owes one of re-seed (`supersedes:`), graft, or decide-abandon (`reconciles:`); the validator warns until it does. Exports are never edited. | `system/LAW.md`, `system/TYPES.md` |
