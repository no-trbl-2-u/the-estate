...
| [0030](0030-the-warm-adopt-path.md) | The warm adopt path: the `build-plan` payload is nexus-native and one `npx … adopt` command fetches the kit around it, leaving no source behind; `seed-check`/`re-seed` live in the kit; `target: nexus \| none` is the switch for "include the Nexus worker loop?", asked of T *(amends 0029 decision 4)* |
| [0031](0031-explicit-entry-via-start.md) | The main session carries no persona by default; entering as The Steward requires invoking `/start` (the `steward` skill is renamed `start`), reversing the standing identity ADR 0019 delivered |
| [0032](0032-the-clean-slate.md) | The clean slate: T wiped all four Idea Records and both exports; the archive is the pushed tag `ideas-archive-2026-09-03`, and ADR 0020 was edited narrowly to unlink a target the wipe removed |
| [0033](0033-projects-are-directories.md) | Projects are directories: `projects/NNNN-slug/` encapsulates its own `ideas/` and `exports/`; scope is location, ids stay global, the index is generated, Seeds colocate with their records; scoring is deliberately deferred |
| [0034](0034-onboard-clerical-intake.md) | `/onboard` is clerical intake for the mature project: a project shell plus supplied material parked verbatim as slips on the project's own `inbox/`; no artifacts, no records, no mid-funnel entry — the funnel is run cheaply over the material *(amends 0033's inbox exclusivity)* |
| [0035](0035-scoped-sessions-and-project-adrs.md) | Scoped sessions and project ADRs: `/start <project>` reads one tree only and bare `/start` asks; projects carry authored `docs/adr/` logs written by `decide` when the subject is the project; the root log is exclusively machinery work outside the Steward context; sibling projects never read each other's ADRs *(supersedes 0033 decision 5 in part)* |
